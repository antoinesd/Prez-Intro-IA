...
Pour résoudre "il" → le modèle attend **fortement** sur "chat"
...
- *Tête 2* : co-références (il/le chat)
- *Tête 3* : relations temporelles (avant/après)
- *Tête N* : ...
...
[NOTE.speaker]
--
La température est l'un des paramètres les plus importants pour contrôler le comportement d'un LLM en production. On voudra généralement T faible pour les sorties structurées (code, données factuelles) et T plus haute pour la génération créative (brainstorming, rédaction).
--
...
[NOTE.speaker]
--
Le pre-training est de loin la phase la plus coûteuse. Le fine-tuning (SFT + RLHF) ne coûte qu'une fraction. C'est pourquoi on peut adapter un LLM à un domaine spécifique (médecine, droit, ingénierie...) avec des ressources raisonnables.
--
...
*TOUJOURS* vérifier les faits, chiffres, dates et citations générés par un LLM — quelle que soit la discipline !
...
[.key-insight]
Une image découpée en patches, une mélodie, une séquence d'ADN, du code source — tous sont des *séquences de tokens* sur lesquelles l'attention opère.

*Ce qui change* : le vocabulaire (pixels vs notes vs mots) et les données d'entraînement.

*Ce qui ne change pas* : le mécanisme d'attention, les blocs Transformer, les scaling laws.

→ *Un seul paradigme pour tout comprendre.*
====
--
